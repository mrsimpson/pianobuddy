...
describe('CollectionService', () => {
  ...
  describe('getCollections', () => {
    it('should fetch available collections', async () => {
      ...
      vi.spyOn(CollectionLoader, 'loadAvailableCollections').mockResolvedValue(mockCollectionIds)
  ...
  describe('getCollection', () => {
    it('should load and cache collection', async () => {
      vi.spyOn(CollectionLoader, 'loadCollection').mockResolvedValue(mockCollection)
      ...
      // Verify caching
      const cachedCollection = CollectionService['collections'].get('christmas')
      expect(cachedCollection).toEqual(mockCollection)
    })

    it('should return cached collection on subsequent calls', async () => {
      const loadSpy = vi.spyOn(CollectionLoader, 'loadCollection').mockResolvedValue(mockCollection)
  ...
  describe('loadCollectionSong', () => {
    it('should load song from collection', async () => {
      ...
      const xmlContent = await CollectionService.loadCollectionSong('christmas', 'song1')

      expect(xmlContent).toBe(mockXmlContent)
      expect(mockFetch).toHaveBeenCalledWith('/collections/christmas/test-song.musicxml')
    })
