...
export class PlaybackService {
  ...
  private playCurrentNote(): void {
    ...
    if (!note.isRest) {
      const durationInSeconds = this._getNoteDurationInSeconds(note.duration.divisions)
      this.audioService.playNote(note.pitch, note.octave, durationInSeconds)
    }

    const nextNoteDelay = this._getNoteDurationInSeconds(note.duration.divisions) * 1000
