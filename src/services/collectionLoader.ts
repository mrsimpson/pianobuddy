...
export class CollectionLoader {
  static async loadCollection(collectionId: string): Promise<MusicCollection> {
    try {
      ...
      const indexResponse = await fetch(
        `/collections/${collectionId}/index.json`,
      );
      if (!indexResponse.ok) {
  ...
  private static async loadMetadata(
    ...
    try {
      const response = await fetch(
        `/collections/${collectionId}/metadata.json`,
      );
      if (!response.ok) {
        throw new Error(
          `Failed to load metadata for collection: ${collectionId}`,
        );
      }
      ...
      console.error(
        `Error loading collection metadata: ${collectionId}`,
        error,
      );
      throw error;
    }
  ...
  private static async loadMusicXmlMetadata(
    xmlContent: string,
  ): Promise<Partial<CollectionSong>> {
    try {
      const doc = parseXml(xmlContent);

      const work = doc.querySelector('work-title')?.textContent || '';
      const composer = doc.querySelector(
        'creator[type="composer"]',
      )?.textContent;
      const arranger = doc.querySelector(
